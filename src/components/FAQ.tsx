...
ملاحظة: رقم العضوية ثابت ولا يمكن تغييره.`
  },
  {
    id: "deposit",
    icon: <Wallet className="w-5 h-5" />,
    question: "كيف يمكنني الإيداع في حسابي؟",
    answer: `لإيداع رصيد في حسابك:

1. ادخل إلى قسم "المحفظة" ثم اضغط "إيداع"
2. حوّل المبلغ إلى رقم المحفظة: 01080048591
3. ارفع صورة إيصال التحويل
4. أدخل رقم العملية المكوّن من 14 رقماً
5. انتظر مراجعة الإدارة (عادة خلال 24 ساعة)

بوابات الدفع المدعومة: فودافون كاش، إتصالات (We)، أورنج كاش.`
  },
  {
    id: "contest",
    icon: <Target className="w-5 h-5" />,
...
• عند إكمال المستوى تحصل على المكافأة المحددة (رصيد، نقاط، أو ترقية مؤقتة)`
  },
  {
    id: "telegram",
    icon: <Shield className="w-5 h-5" />,
    question: "لماذا أحتاج تفعيل بوت التليجرام؟",
    answer: `بوت التليجرام @Advance0bot ضروري لأنه:

• يُرسل لك رموز التحقق (OTP) عند تسجيل الدخول
• يحمي العمليات الحساسة مثل السحب وتغيير البيانات
• كل حساب مرتبط ببوت واحد فقط (حساب واحد لكل تليجرام)
• يضمن أن أنت صاحب الحساب الفعلي عند أي إجراء مهم

للتفعيل: افتح البوت في تليجرام واضغط "Start" ثم اتبع التعليمات داخل التطبيق.`
  },
  {
    id: "pwa",
    icon: <Zap className="w-5 h-5" />,
    question: "كيف أثبّت التطبيق على هاتفي وأحصل على مكافأة 20 جنيه؟",
    answer: `يمكنك تثبيت Advance كتطبيق حقيقي على هاتفك:

1. على أندرويد: افتح Chrome واضغط على "إضافة إلى الشاشة الرئيسية" من قائمة المتصفح
2. على iPhone: افتح Safari ثم اضغط زر المشاركة ثم "أضف إلى الشاشة الرئيسية"
3. سيظهر الشريط الأخضر داخل التطبيق لتأكيد التثبيت
4. بمجرد التثبيت الناجح تُضاف 20 جنيه تلقائياً لرصيدك (مرة واحدة فقط)`
  },
  {
    id: "ai-support",
    icon: <MessageSquare className="w-5 h-5" />,
...
• محاولة استخدام أكثر من حساب على نفس الجهاز/التليجرام
• إدخال بيانات سحب وهمية أو مزيفة
• مخالفة شروط الاستخدام

للاستفسار أو الطعن:
1. تواصل مع بوت التليجرام @Advance0bot
