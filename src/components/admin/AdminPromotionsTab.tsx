...
export const AdminPromotionsTab = () => {
  ...
  const handleEdit = (promo: Promotion) => {
    ...
    setLinkType(promo.link_type);
    setDisplayOrder(promo.display_order);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card className="border-border/50">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-accent">{promotions.filter(p => p.is_active).length}</p>
            <p className="text-xs text-muted-foreground">نشط</p>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-muted-foreground">{promotions.filter(p => !p.is_active).length}</p>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="/app/wallet أو https://..."
              />
            </div>
                ...
                {promotions.map(promo => (
                  <motion.div
                    ...
                    className={`p-4 rounded-xl border ${
                      promo.is_active ? 'bg-primary/5 border-primary/20' : 'bg-secondary/20 border-border/50'
                    }`}
                  >
